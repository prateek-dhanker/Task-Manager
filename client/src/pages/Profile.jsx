...
const Profile = (props) => {
  ...
  useEffect(() => {
    axios.get(`${process.env.VITE_API_URL}/api/auth/all`)
      .then(res => {
  ...
  const handleDelete = async(username) =>{
    try {
      if(confirm("wanna delete")){
        const res = await axios.post(`${process.env.VITE_API_URL}/api/auth/delete`, {username });
  ...
  const handleEdit = async(username) =>{
    try {
      ...
      if(newUsername.length){
        const res = await axios.post(`${process.env.VITE_API_URL}/api/auth/edit`, {username : username, newUsername:newUsername, admin:token.admin});
