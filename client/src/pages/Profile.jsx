...
const Profile = (props) => {
  ...
  useEffect(() => {
    axios.get('https://task-manager-v0ym.onrender.com/api/auth/all')
      .then(res => {
  ...
  const handleDelete = async(username) =>{
    try {
      if(confirm("wanna delete")){
        const res = await axios.post('https://task-manager-v0ym.onrender.com/api/auth/delete', {username });
  ...
  const handleEdit = async(username) =>{
    try {
      ...
      if(newUsername.length){
        const res = await axios.post('https://task-manager-v0ym.onrender.com/api/auth/edit', {username : username, newUsername:newUsername, admin:token.admin});
