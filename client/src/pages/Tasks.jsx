...
const Tasks = (props) => {
  ...
  const handleSubmit = async(e) => {
    ...
    if(formData.id){
      try {
        const res = await axios.post('http://localhost:3000/api/tasks/delete', {title:oldData.title,assignedTo:oldData.assignedTo});
      } catch (err) {
    ...
    try {
      const res = await axios.post('http://localhost:3000/api/tasks/add', form, {
  ...
  const handleDelete = async(title,assignedTo) => {
    try {
      if(confirm("wanna delete")){
        const res = await axios.post('http://localhost:3000/api/tasks/delete', {title, assignedTo });    
      }
    } catch (err) {
